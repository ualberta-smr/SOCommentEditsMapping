...
def get_data(conn):
    df_answers = pd.read_sql_query(
        "SELECT * FROM EditHistory_Code WHERE Event = 'InitialBody' AND PostId IN (24548749);",
        conn, parse_dates={"CreationDate": "%Y-%m-%d %H:%M:%S"})
    df_comments = pd.read_sql_query(
        "SELECT * FROM EditHistory_Code WHERE Event = 'Comment' AND PostId IN (24548749);",
        conn, parse_dates={"CreationDate": "%Y-%m-%d %H:%M:%S"})
    df_edits = pd.read_sql_query(
        "SELECT * FROM EditHistory_Code WHERE Event = 'BodyEdit' AND PostId IN (24548749);",
        conn, parse_dates={"CreationDate": "%Y-%m-%d %H:%M:%S"})
    df_questions = pd.read_sql_query(
        "SELECT * FROM EditHistory WHERE Event = 'InitialBody' AND PostId IN (SELECT DISTINCT ParentId FROM EditHistory WHERE PostId IN (24548749));",
        conn, parse_dates={"CreationDate": "%Y-%m-%d %H:%M:%S"})
